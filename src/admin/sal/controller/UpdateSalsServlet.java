...
/**
 * Servlet implementation class UpdateSalsServlet
 */
@WebServlet("/UpdateSalsServlet")
public class UpdateSalsServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public UpdateSalsServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int order_detail_no_int=0;
		String order_detail_no = request.getParameter("order_no");
		System.out.println("받아온 값????"+order_detail_no);
		if(order_detail_no != null) {
			order_detail_no_int = Integer.parseInt(order_detail_no);
			
			int result= new adminSalService().updateOrder(order_detail_no_int);
			...
			if(result==1) {
				...
				//response.sendRedirect("./WEB-INF/view/selectSal.jsp");
			}
	//		else {
	//			request.getRequestDispatcher("./WEB-INF/view/selectSal.jsp").forward(request, response);
	//		}
